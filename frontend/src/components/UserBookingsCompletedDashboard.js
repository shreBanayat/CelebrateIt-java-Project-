...
import { BookingCompleteCard } from "./BookingCompleteCard";
...
function UserBookingsCompletedDashboard() {
  ...
  return (
    <div className="user-bookings-dashboard">
       ...
       <h1 className="booking-details-title">Completed Booking Details</h1>
          ...
          <div key={id} className={`category-section ${categoryName.toLowerCase().replace(/\s/g, "-")}`}>
            ...
            {selectedCategoryId === id && (
              <div className="services-container">
                <BookingCompleteCard categoryId={id} />
              </div>
            )}
          </div>
        );
...
export default UserBookingsCompletedDashboard;
